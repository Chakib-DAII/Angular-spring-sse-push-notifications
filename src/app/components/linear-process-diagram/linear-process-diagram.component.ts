import { Component, OnInit } from '@angular/core';
import * as am4core from "@amcharts/amcharts4/core";
import * as am4charts from "@amcharts/amcharts4/charts";
import * as am4plugins_timeline from "@amcharts/amcharts4/plugins/timeline"
import am4themes_animated from "@amcharts/amcharts4/themes/animated";
import * as am4plugins_bullets from "@amcharts/amcharts4/plugins/bullets";

am4core.useTheme(am4themes_animated);

@Component({
  selector: 'app-linear-process-diagram',
  templateUrl: './linear-process-diagram.component.html',
  styleUrls: ['./linear-process-diagram.component.scss']
})
export class LinearProcessDiagramComponent implements OnInit {

  private chart: am4plugins_timeline.CurveChart;
  private colorSet: any;
  private categoryAxis : any;
  private dateAxis : any;
  private series: any;
  private labelTemplate: any;
  private imageBullet1: any;
  private previousBullet: any;
  private cursor: any;
  private hs: any;
  private textBullet: any;

  chartData: any[];

private alarm = "data:image/svg+xml;base64,";
private water = "data:image/svg+xml;base64,";
private exercise = "data:image/svg+xml;base64,";
private breakfast = "data:image/svg+xml;base64,";
private work = "data:image/svg+xml;base64,";
private car = "data:image/svg+xml;base64,";
private coffee = "data:image/svg+xml;base64,";
private dinner = "data:image/svg+xml;base64,";
private book = "data:image/svg+xml;base64,";
private home = "data:image/svg+xml;base64,";
private beer = "data:image/svg+xml;base64,";
private dance = "data:image/svg+xml;base64,";
private drink = "data:image/svg+xml;base64,";
private drunk = "data:image/svg+xml;base64,";
private bed = "data:image/svg+xml;base64,";


  constructor() { }

  ngOnInit(): void {

    this.chart = am4core.create("chartdiv", am4plugins_timeline.CurveChart);
    this.chart.curveContainer.padding(100, 20, 50, 20);
    this.chart.maskBullets = false;

    this.colorSet = new am4core.ColorSet();

    this.chart.dateFormatter.inputDateFormat = "yyyy-MM-dd HH:mm";
    this.chart.dateFormatter.dateFormat = "HH";

    this.chart.data = [{
      "category": "",
      "start": "2019-01-10 06:00",
      "end": "2019-01-10 06:15",
      "color": this.colorSet.getIndex(15),
      "icon": this.alarm,
      "text": "Wake up!"
  }, {
      "category": "",
      "start": "2019-01-10 06:15",
      "end": "2019-01-10 06:30",
      "color": this.colorSet.getIndex(14),
      "icon": this.water,
      "text": "Drink water"
  },
  {
      "category": "",
      "start": "2019-01-10 06:30",
      "end": "2019-01-10 07:00",
      "color": this.colorSet.getIndex(13),
      "icon": this.exercise,
      "text": "Exercise"
  },
  {
      "category": "",
      "start": "2019-01-10 07:00",
      "end": "2019-01-10 07:30",
      "color": this.colorSet.getIndex(12),
      "icon": this.breakfast,
      "text": "Have breakfast"
  },
  {
      "category": "",
      "start": "2019-01-10 07:30",
      "end": "2019-01-10 08:00",
      "color": this.colorSet.getIndex(11),
      "icon": this.car,
      "text": "Drive to work"
  },
  {
      "category": "",
      "start": "2019-01-10 08:00",
      "end": "2019-01-10 17:00",
      "color": this.colorSet.getIndex(10),
      "icon": this.work,
      "text": "Work"
  },
  {
      "category": "e",
      "start": "2019-01-10 10:00",
      "end": "2019-01-10 10:15",
      "color": this.colorSet.getIndex(10),
      "icon": this.coffee,
      "text": "Coffee"
  },
  {
      "category": "e",
      "start": "2019-01-10 12:00",
      "end": "2019-01-10 13:00",
      "color": this.colorSet.getIndex(10),
      "icon": this.dinner,
      "text": "Dinner"
  },
  {
      "category": "e",
      "start": "2019-01-10 14:00",
      "end": "2019-01-10 14:15",
      "color": this.colorSet.getIndex(10),
      "icon": this.coffee,
      "text": "Coffee"
  },
  {
      "category": "",
      "start": "2019-01-10 17:00",
      "end": "2019-01-10 18:00",
      "color": this.colorSet.getIndex(8),
      "icon": this.car,
      "text": "Drive home"
  },
  {
      "category": "",
      "start": "2019-01-10 18:00",
      "end": "2019-01-10 21:30",
      "color": this.colorSet.getIndex(7),
      "icon": this.home,
      "text": "Home!"
  },
  {
      "category": "e",
      "start": "2019-01-10 19:30",
      "end": "2019-01-10 20:30",
      "color": this.colorSet.getIndex(7),
      "icon": this.book,
      "text": "Read a bit"
  },
  {
      "category": "",
      "start": "2019-01-10 21:30",
      "end": "2019-01-10 22:00",
      "color": this.colorSet.getIndex(6),
      "icon": this.beer,
      "text": "Have a beer"
  },
  {
      "category": "",
      "start": "2019-01-10 22:00",
      "end": "2019-01-10 22:15",
      "color": this.colorSet.getIndex(5),
      "icon": this.beer,
      "text": "Have another beer"
  },
  {
      "category": "",
      "start": "2019-01-10 22:15",
      "end": "2019-01-10 23:00",
      "color": this.colorSet.getIndex(4),
      "icon": this.dance,
      "text": "Dance!"
  },
  {
      "category": "",
      "start": "2019-01-10 23:00",
      "end": "2019-01-11 00:00",
      "color": this.colorSet.getIndex(3),
      "icon": this.drink,
      "text": "Martini!"
  },
  {
      "category": "",
      "start": "2019-01-11 00:00",
      "end": "2019-01-11 01:00",
      "color": this.colorSet.getIndex(2),
      "icon": this.drunk,
      "text": "Damn..."
  },
  {
      "category": "",
      "start": "2019-01-11 01:00",
      "end": "2019-01-11 01:00",
      "color": this.colorSet.getIndex(1),
      "icon": this.bed,
      "text": "Bye bye"
  }];

  this.chart.fontSize = 10;
  this.chart.tooltipContainer.fontSize = 10;

  this.categoryAxis = this.chart.yAxes.push(new am4charts.CategoryAxis() as any);
  this.categoryAxis.dataFields.category = "category";
  this.categoryAxis.renderer.grid.template.disabled = true;
  this.categoryAxis.renderer.labels.template.paddingRight = 25;
  this.categoryAxis.renderer.minGridDistance = 10;
  this.categoryAxis.renderer.innerRadius = 10;
  this.categoryAxis.renderer.radius = 30;

  this.dateAxis = this.chart.xAxes.push(new am4charts.DateAxis() as any);

  this.dateAxis.renderer.points = getPoints();


  this.dateAxis.renderer.autoScale = false;
  this.dateAxis.renderer.autoCenter = false;
  this.dateAxis.renderer.minGridDistance = 70;
  this.dateAxis.baseInterval = { count: 5, timeUnit: "minute" };
  this.dateAxis.renderer.tooltipLocation = 0;
  this.dateAxis.renderer.line.strokeDasharray = "1,4";
  this.dateAxis.renderer.line.strokeOpacity = 0.5;
  this.dateAxis.tooltip.background.fillOpacity = 0.2;
  this.dateAxis.tooltip.background.cornerRadius = 5;
  this.dateAxis.tooltip.label.fill = new am4core.InterfaceColorSet().getFor("alternativeBackground");
  this.dateAxis.tooltip.label.paddingTop = 7;
  this.dateAxis.endLocation = 0;
  this.dateAxis.startLocation = -0.5;
  this.dateAxis.min = new Date(2019, 0, 9, 23, 55).getTime();
  this.dateAxis.max = new Date(2019, 0, 11, 7, 10).getTime();

  this.labelTemplate = this.dateAxis.renderer.labels.template;
  this.labelTemplate.verticalCenter = "middle";
  this.labelTemplate.fillOpacity = 0.6;
  this.labelTemplate.background.fill = new am4core.InterfaceColorSet().getFor("background");
  this.labelTemplate.background.fillOpacity = 1;
  this.labelTemplate.fill = new am4core.InterfaceColorSet().getFor("text");
  this.labelTemplate.padding(7, 7, 7, 7);

  this.series = this.chart.series.push(new am4plugins_timeline.CurveColumnSeries());
  this.series.columns.template.height = am4core.percent(30);

  this.series.dataFields.openDateX = "start";
  this.series.dataFields.dateX = "end";
  this.series.dataFields.categoryY = "category";
  this.series.baseAxis = this.categoryAxis;
  this.series.columns.template.propertyFields.fill = "color"; // get color from data
  this.series.columns.template.propertyFields.stroke = "color";
  this.series.columns.template.strokeOpacity = 0;
  this.series.columns.template.fillOpacity = 0.6;

  this.imageBullet1 = this.series.bullets.push(new am4plugins_bullets.PinBullet());
  this.imageBullet1.background.radius = 18;
  this.imageBullet1.locationX = 1;
  this.imageBullet1.propertyFields.stroke = "color";
  this.imageBullet1.background.propertyFields.fill = "color";
  this.imageBullet1.image = new am4core.Image();
  this.imageBullet1.image.propertyFields.href = "icon";
  this.imageBullet1.image.scale = 0.7;
  this.imageBullet1.circle.radius = am4core.percent(100);
  this.imageBullet1.background.fillOpacity = 0.8;
  this.imageBullet1.background.strokeOpacity = 0;
  this.imageBullet1.dy = -2;
  this.imageBullet1.background.pointerBaseWidth = 10;
  this.imageBullet1.background.pointerLength = 10
  this.imageBullet1.tooltipText = "{text}";

  this.series.tooltip.pointerOrientation = "up";

  this.imageBullet1.background.adapter.add("pointerAngle", (value, target) => {
    if (target.dataItem) {
        let position = this.dateAxis.valueToPosition(target.dataItem.openDateX.getTime());
        return this.dateAxis.renderer.positionToAngle(position);
    }
    return value;
  });

  this.hs = this.imageBullet1.states.create("hover")
  this.hs.properties.scale = 1.3;
  this.hs.properties.opacity = 1;

  this.textBullet = this.series.bullets.push(new am4charts.LabelBullet());
  this.textBullet.label.propertyFields.text = "text";
  this.textBullet.disabled = true;
  this.textBullet.propertyFields.disabled = "textDisabled";
  this.textBullet.label.strokeOpacity = 0;
  this.textBullet.locationX = 1;
  this.textBullet.dy = - 100;
  this.textBullet.label.textAlign = "middle";

  this.chart.scrollbarX = new am4core.Scrollbar();
  this.chart.scrollbarX.align = "center"
  this.chart.scrollbarX.width = am4core.percent(75);
  this.chart.scrollbarX.parent = this.chart.curveContainer;
  this.chart.scrollbarX.height = 300;
  this.chart.scrollbarX.orientation = "vertical";
  this.chart.scrollbarX.x = 128;
  this.chart.scrollbarX.y = -140;
  this.chart.scrollbarX.isMeasured = false;
  this.chart.scrollbarX.opacity = 0.5;

  this.cursor = new am4plugins_timeline.CurveCursor();
  this.chart.cursor = this.cursor;
  this.cursor.xAxis = this.dateAxis;
  this.cursor.yAxis = this.categoryAxis;
  this.cursor.lineY.disabled = true;
  this.cursor.lineX.disabled = true;

  this.dateAxis.renderer.tooltipLocation2 = 0;
  this.categoryAxis.cursorTooltipEnabled = false;

  this.chart.zoomOutButton.disabled = true;

  //this.previousBullet;

  this.chart.events.on("inited", function() {
      setTimeout(function() {
          hoverItem(this.series.dataItems.getIndex(0));
      }, 2000)
  });

  function hoverItem(dataItem) {
      let bullet = dataItem.bullets.getKey(this.imageBullet1.uid);
      let index = dataItem.index;

      if (index >= this.series.dataItems.length - 1) {
          index = -1;
      }

      if (bullet) {

          if (this.previousBullet) {
            this.previousBullet.isHover = false;
          }

          bullet.isHover = true;

          this.previousBullet = bullet;
      }
      setTimeout(
          function() {
              hoverItem(this.series.dataItems.getIndex(index + 1))
          }, 1000);
    }

  function getPoints() {

  let points = [{ x: -1300, y: 200 }, { x: 0, y: 200 }];

  let w = 400;
  let h = 400;
  let levelCount = 4;

  let radius = am4core.math.min(w / (levelCount - 1) / 2, h / 2);
  let startX = radius;

  for (var i = 0; i < 25; i++) {
      let angle = 0 + i / 25 * 90;
      let centerPoint = { y: 200 - radius, x: 0 }
      points.push({ y: centerPoint.y + radius * am4core.math.cos(angle), x: centerPoint.x + radius * am4core.math.sin(angle) });
  }


  for (var i = 0; i < levelCount; i++) {

      if (i % 2 != 0) {
          points.push({ y: -h / 2 + radius, x: startX + w / (levelCount - 1) * i })
          points.push({ y: h / 2 - radius, x: startX + w / (levelCount - 1) * i })

          let centerPoint = { y: h / 2 - radius, x: startX + w / (levelCount - 1) * (i + 0.5) }
          if (i < levelCount - 1) {
              for (var k = 0; k < 50; k++) {
                  let angle = -90 + k / 50 * 180;
                  points.push({ y: centerPoint.y + radius * am4core.math.cos(angle), x: centerPoint.x + radius * am4core.math.sin(angle) });
              }
          }

          if (i == levelCount - 1) {
              points.pop();
              points.push({ y: -radius, x: startX + w / (levelCount - 1) * i })
              let centerPoint = { y: -radius, x: startX + w / (levelCount - 1) * (i + 0.5) }
              for (var k = 0; k < 25; k++) {
                  let angle = -90 + k / 25 * 90;
                  points.push({ y: centerPoint.y + radius * am4core.math.cos(angle), x: centerPoint.x + radius * am4core.math.sin(angle) });
              }
              points.push({ y: 0, x: 1300 });
          }

      }
      else {
          points.push({ y: h / 2 - radius, x: startX + w / (levelCount - 1) * i })
          points.push({ y: -h / 2 + radius, x: startX + w / (levelCount - 1) * i })
          let centerPoint = { y: -h / 2 + radius, x: startX + w / (levelCount - 1) * (i + 0.5) }
          if (i < levelCount - 1) {
              for (var k = 0; k < 50; k++) {
                  let angle = -90 - k / 50 * 180;
                  points.push({ y: centerPoint.y + radius * am4core.math.cos(angle), x: centerPoint.x + radius * am4core.math.sin(angle) });
              }
          }
      }
  }
  return points;
  }
  }
}
